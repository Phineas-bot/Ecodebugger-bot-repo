...
function someFunction() {
    ...
    const result = globalXpEngine.processCodeSave(code);
    const report = globalXpEngine.getFormattedReport(result);
...
vscode.workspace.onDidSaveTextDocument((doc) => {
    if (ecoTipsEnabled && (doc.languageId === 'javascript' || doc.languageId === 'typescript' || doc.languageId === 'python')) {
        provideEcoTips();
        // Use the XP engine for processing saves
...
export function activate(context: vscode.ExtensionContext): void {
      ...
      // Initialize local variables from globalXpEngine
      xp = globalXpEngine.getCurrentXP();
      level = globalXpEngine.getCurrentLevel();
    ...
    const openEcoDebuggerUI = vscode.commands.registerCommand('ecoDebugger.openUI', () => {
        const panel = vscode.window.createWebviewPanel(
            ...
            // Initial state - use current values from globalXpEngine
        const state = {
            xp: globalXpEngine.getCurrentXP(),
            level: globalXpEngine.getCurrentLevel(),
            xpLog,
