...
function analyzeGreenCode(text) {
    ...
    if (nestedLoopDetected) {
        vscode.window.showWarningMessage('⚡ Eco Tip: Avoid nested loops. Consider using more efficient algorithms or data structures.');
    }
    if (inefficientStringConcat) {
        vscode.window.showWarningMessage('⚡ Eco Tip: Avoid string concatenation in loops. Use Array.join() or template literals.');
    }
    if (redundantCalculationDetected) {
        vscode.window.showWarningMessage('⚡ Eco Tip: Avoid redundant calculations. Store results in a variable if reused.');
    }
    if (unusedVariables.length > 0) {
        vscode.window.showWarningMessage(`⚡ Eco Tip: Remove unused variables (${unusedVariables.join(', ')}) to improve code efficiency.`);
    }
    if (inefficientArrayPush) {
        vscode.window.showWarningMessage('⚡ Eco Tip: Avoid using .push() in loops for large arrays. Pre-allocate or use Array methods.');
    }
}
